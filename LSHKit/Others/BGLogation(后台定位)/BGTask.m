...
-(UIBackgroundTaskIdentifier)beginNewBackgroundTask
{
    ...
    if([application respondsToSelector:@selector(beginBackgroundTaskWithExpirationHandler:)])
    {
        bgTaskId = [application beginBackgroundTaskWithExpirationHandler:^{
            ...
            NSString *str = [NSString stringWithFormat:@"过期后台任务--->%lu",(unsigned long)bgTaskId];
            [[DBManager shareDB] insertLogWithPage:NSStringFromClass([self class]) Content:str Date:[NSDate date]];
            [self.bgTaskIdList removeObject:@(bgTaskId)];//过期任务从后台数组删除
           
            [application endBackgroundTask:bgTaskId];
            bgTaskId = UIBackgroundTaskInvalid;
        }];
        NSLog(@"新建后台任务--->%lu",(unsigned long)bgTaskId);
        NSString *str = [NSString stringWithFormat:@"新建后台任务--->%lu",(unsigned long)bgTaskId];
        [[DBManager shareDB] insertLogWithPage:NSStringFromClass([self class]) Content:str Date:[NSDate date]];
    }
